  ...
  function handleSubmit(e) {
    ...
    if (accepted.includes(answer)) {
      ...
      if (retryCount >= MAX_RETRIES) {
        ...
        setFeedback('hint')
        setTimeout(() => { setFeedback(null); setInput('') }, 1500)
      }
    }
  }
    ...
    return (
      <div className="min-h-screen bg-navy-950 flex flex-col items-center justify-center px-6">
        <div className="max-w-lg text-center">
          {/* Stage count */}
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-teal-400/10 border border-teal-400/20 rounded-full text-teal-400 text-sm font-semibold mb-8">
            <div className="w-2 h-2 rounded-full bg-teal-400 animate-pulse" />
            {questionBank.stageCount} Stages
          </div>

          {/* Theorem name */}
          <h1 className="font-display text-4xl sm:text-5xl text-cream-100 font-bold mb-6">
            {questionBank.displayName || theoremData.theorem}
          </h1>

          {/* Story intro */}
          <div className="bg-navy-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 mb-8 text-left">
          ...
          {/* Exit link */}
          <button onClick={onExit} className="block mx-auto mt-6 text-cream-400 text-sm hover:text-teal-400 transition-colors">
            ← Back to Dashboard
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-navy-950 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        <div className="w-full max-w-2xl space-y-8">
          ...
          {/* Answer input */}
          <form onSubmit={handleSubmit} className={`relative ${feedback === 'hint' ? 'animate-shake' : ''}`}>
            <input
              ref={inputRef}
          ...
          {/* Hint slide-in */}
          <div className={`space-y-3 overflow-hidden transition-all duration-500 ${currentHint ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="flex items-start gap-3 bg-coral-400/10 border border-coral-400/20 rounded-2xl p-5">
              ...
              <div>
                ...
                <p className="text-xs text-coral-400/60 mt-2 font-mono">Attempts: {retryCount}/{MAX_RETRIES}</p>
              </div>
            </div>
          </div>
