...
// KEY FIX: embed view inside session for atomic persistence
        ...
        {session ? (
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={onSignOut} className="p-2.5 hover:bg-coral-400/10 text-coral-400 rounded-xl transition-all group">
...
function Register({ onSwitch, onSuccess }) {
  ...
  return (
    <div className="w-full max-w-md pt-24">
      ...
      <div className="bg-navy-900/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
        {step === 1 && (
          <div className="flex flex-col gap-5">
            ...
            <button onClick={handleSendOTP} disabled={status === 'loading'} className="w-full py-3.5 bg-teal-400 hover:bg-teal-300 text-navy-950 rounded-xl font-bold transition-all shadow-lg shadow-teal-400/20">
        ...
        {step === 2 && (
          <div className="flex flex-col gap-6">
            ...
            <button onClick={handleVerify} disabled={status === 'loading'} className="w-full py-3.5 bg-teal-400 hover:bg-teal-300 text-navy-950 rounded-xl font-bold shadow-lg shadow-teal-400/20">
        ...
        {step === 3 && (
          <div className="flex flex-col gap-5">
            ...
            <button onClick={handleRegister} disabled={status === 'loading'} className="w-full py-3.5 bg-teal-400 hover:bg-teal-300 text-navy-950 rounded-xl font-bold shadow-lg shadow-teal-400/20">
      ...
      <p className="text-center text-cream-300/60 text-sm mt-6 font-sans">Already have an account? <button onClick={onSwitch} className="text-teal-400 font-semibold hover:underline">Sign in</button></p>
    </div>
  )
}
  ...
  return (
    <div className="w-full max-w-md pt-24">
      ...
      <div className="bg-navy-900/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl flex flex-col gap-5">
        ...
        <button onClick={handleLogin} disabled={status === 'loading'} className="w-full py-3.5 bg-teal-400 hover:bg-teal-300 text-navy-950 rounded-xl font-bold transition-all shadow-lg shadow-teal-400/20">
          {status === 'loading' ? 'Signing in...' : 'Sign In'}
        </button>
        {status === 'error' && message && <p className="text-coral-400 text-sm text-center">{message}</p>}
      </div>
      <p className="text-center text-cream-300/60 text-sm mt-6 font-sans">Don't have an account? <button onClick={onSwitch} className="text-teal-400 font-semibold hover:underline">Create one</button></p>
    </div>
  )
}
  ...
  return (
    <div className="w-full max-w-md pt-24">
      ...
      <div className="bg-navy-900/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
        {step === 1 && (
          <div className="flex flex-col gap-5">
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Enter your email" className="w-full px-4 py-3 bg-navy-800/50 border-2 border-navy-700 rounded-lg text-cream-100 outline-none focus:border-teal-400" />
...
// ─── MAIN APP ──────────────────────────────────────
// KEY FIX: view stored INSIDE session object so they persist together atomically
// No separate localStorage key for view — it's all one object
...
export default function App() {
  ...
  // View state: derived from session when logged in, falls back to localStorage math_view
  // This avoids the issue where math_view gets stuck and causes wrong page on refresh
  ...
  // Update view by modifying session object atomically (when session exists)
      ...
      {view === 'home' && !session && (
        <div className="text-center pt-40 px-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={() => setCurrentView('register')} className="px-12 py-4 bg-teal-400 text-navy-950 font-bold rounded-xl hover:bg-teal-300 hover:scale-105 transition-all shadow-xl shadow-teal-400/20">Create Account</button>
            <button onClick={() => setCurrentView('login')} className="px-12 py-4 border-2 border-teal-400 text-teal-400 font-bold rounded-xl hover:bg-teal-400/10 hover:scale-105 transition-all">Sign In</button>
          </div>
        </div>
      )}
