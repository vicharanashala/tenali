...
/**
 * TeacherLogin.jsx — Teacher login page (/teacher/login)
...
export default function TeacherLogin() {
  ...
  useEffect(() => {
    if (!import.meta.env.VITE_GOOGLE_CLIENT_ID) {
      console.warn('[TeacherLogin] VITE_GOOGLE_CLIENT_ID not set')
      return
    }
  ...
  const retryWithCookieBased = () => {
    console.log('[TeacherLogin] FedCM blocked — retrying with cookie-based flow...')
    ...
    window.google.accounts.id.initialize({
      ...
      callback: async (response) => {
        console.log('[TeacherLogin] Cookie-based callback — response:', response)
        initialized.current = false
        if (!response.credential) { setError('No credential returned.'); setLoading(false); return }
        try {
          ...
          console.log('[TeacherLogin] Cookie-based API response:', res.status, data)
          ...
          console.log('[TeacherLogin] Cookie-based SUCCESS, redirect:', data.user.role === 'teacher' ? '/teacher/dashboard' : '/dashboard')
  ...
  const handleGoogleSignIn = () => {
    console.log('[TeacherLogin] Button clicked')
    if (!window.google?.accounts) {
      console.error('[TeacherLogin] FAIL: Google SDK not loaded')
      setError('Google Sign-In is not ready. Please refresh the page.')
      return
    }
    if (initialized.current) { console.warn('[TeacherLogin] Blocked: already initialized'); return }
    initialized.current = true
    setLoading(true)
    setError('')
    ...
    window.google.accounts.id.initialize({
      ...
      callback: async (response) => {
        console.log('[TeacherLogin] Callback fired — response:', response)
        initialized.current = false
        if (!response.credential) { console.error('[TeacherLogin] FAIL: No credential'); setError('No credential returned.'); setLoading(false); return }
        console.log('[TeacherLogin] Credential received, calling /api/auth/google-login...')
        try {
          const res = await fetch('/api/auth/google-login', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ credential: response.credential, role: 'teacher' }),
          ...
          const data = await res.json()
          console.log('[TeacherLogin] API response:', res.status, data)
          if (!res.ok) { setError(data.message || 'Login failed.'); setLoading(false); return }
          const session = { userId: data.user.id, email: data.user.email, name: data.user.name, role: data.user.role || 'teacher', createdAt: Date.now(), view: 'dashboard', method: 'google' }
          localStorage.setItem('tenali_session', JSON.stringify(session))
          console.log('[TeacherLogin] Session stored, redirect:', data.user.role === 'teacher' ? '/teacher/dashboard' : '/dashboard')
          window.location.href = data.user.role === 'teacher' ? '/teacher/dashboard' : '/dashboard'
        } catch (err) { console.error('[TeacherLogin] Fetch error:', err); setError('Could not connect to server.'); setLoading(false) }
      },
    })

    window.google.accounts.id.prompt((notification) => {
      if (!notification) { console.warn('[TeacherLogin] FedCM: null notification'); return }
      const notDisplayed = notification.isNotDisplayed?.()
      const skipped = notification.isSkippedMoment?.()
      const optOut = notification.optOutIsDisplayed?.()
      console.warn('[TeacherLogin] FedCM notification:', { notDisplayed, skipped, optOut })
      if (notDisplayed || skipped || optOut) {
        console.error('[TeacherLogin] FedCM BLOCKED — triggering fallback')
      ...
      <div className="w-full max-w-md">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-cream-400 text-sm">Sign in to manage your students</p>
          </div>
        ...
        <p className="text-center text-cream-400 text-sm mt-6">New to Tenali?{' '}<button onClick={() => navigate('/teacher/register')} className="text-amber-400 font-semibold hover:underline">Create teacher account</button></p>
      </div>
    </div>
  )
}