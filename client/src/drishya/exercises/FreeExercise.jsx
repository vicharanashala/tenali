...
export default function FreeExercise({
  items,
  onAnswer,
  onBack,
  masteryState,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('Error checking FreeDescription answer:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const freeDesc = currentItem.freeDescription || {};
  const coreKeywords = freeDesc.keywords?.core || [];
  const bonusKeywords = freeDesc.keywords?.bonus || [];
  const minWords = freeDesc.minWords || 0;
  const requiresVerb = freeDesc.requiresVerb || false;
    ...
    <div style={{ maxWidth: '650px', margin: '0 auto' }}>
      {!isLevelMode && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '1.4rem', color: 'var(--clr-accent)' }}>Free Description</h2>
          </div>
          <button
            onClick={onBack}
            style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ color: 'var(--clr-text-soft)' }}>
            📏 Min Words: <strong style={{ color: 'var(--clr-text)' }}>{minWords}</strong>
          </span>
          <span style={{ color: 'var(--clr-text-soft)' }}>
            🏃 Action Word: <strong style={{ color: 'var(--clr-text)' }}>{requiresVerb ? 'Required' : 'Optional'}</strong>
          </span>
        </div>
        ...
        <div style={{ width: '100%', position: 'relative' }}>
          <textarea
            ...
            disabled={checked}
            rows={4}
            placeholder="Write a detailed description of what you see in the image..."
            style={{
              width: '100%',
