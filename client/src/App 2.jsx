...
function QuadraticApp({ onBack }) {
  ...
  // Quiz finished?
  const [finished, setFinished] = useState(false)
  // Current question: {a, b, c, x, answer}
  const [question, setQuestion] = useState(null)
  // User's numeric answer
  const [answer, setAnswer] = useState('')
  // Correct answer count
  ...
  // Total questions
  const [totalQ, setTotalQ] = useState(DEFAULT_TOTAL)
  // Feedback with step-by-step working
  const [feedback, setFeedback] = useState('')
  // Is answer correct?
  const [isCorrect, setIsCorrect] = useState(null)
  // Fetching?
  ...
  /**
   * fetchQuestion(selectedDifficulty?): Fetch next quadratic substitution question
   * Generates random coefficients (a, b, c) and value (x), asks for y = ax² + bx + c
   */
 ...
 * Multiplication tables practice with flexible table selection (2-9).
 * User selects which tables to practice, and questions are shuffled from that pool.
 * Allows limiting to a specific number of questions instead of all.
 *
 * @param {Object} props
 * @param {Function} props.onBack - Callback to return to home menu
 */
function MultiplyApp({ onBack }) {
  // Array of selected table numbers (e.g., [2, 3, 5])
  const [selectedTables, setSelectedTables] = useState([])
  // User-entered question limit (empty = all)
  const [numQuestions, setNumQuestions] = useState('')
  // Quiz started?
  const [started, setStarted] = useState(false)
  // Quiz finished?
  const [finished, setFinished] = useState(false)
  // Current question: {table, multiplier, prompt}
  const [question, setQuestion] = useState(null)
  // User's numeric answer
  const [answer, setAnswer] = useState('')
  // Correct answer count
  ...
  // Results array
  const [results, setResults] = useState([])
  // Pre-generated pool of all questions (shuffled)
  ...
  /**
   * toggleTable(num): Add or remove table from selection
   */
  ...
  /**
   * buildPool(tables): Generate and shuffle pool of multiplication questions
   * For each table, creates 10 questions (table × 1 through table × 10)
   * Then shuffles the entire pool for random order
   */
  const buildPool = (tables) => {
    const pool = []
    tables.forEach((t) => {
      // Add all 10 multipliers for this table
      for (let m = 1; m <= 10; m++) {
  ...
  /**
   * startQuiz(): Initialize quiz with selected tables and question count
   * Builds shuffled question pool and loads first question
   */
  const startQuiz = () => {
    if (selectedTables.length === 0) return
    // Build pool of all possible questions from selected tables
    const pool = buildPool(selectedTables)
    // Limit to user-specified count (or all if empty)
...
/**
 * VocabApp Component
 * Vocabulary definition matching quiz: "What does word X mean?"
 * Multiple-choice from 4 definition options (A, B, C, D).
 * Difficulty levels: easy, medium, hard (different vocabulary levels)
 * Questions are not repeated (tracks seenIds).
 *
 * @param {Object} props
 * @param {Function} props.onBack - Callback to return to home menu
 */
function VocabApp({ onBack }) {
  ...
  // Quiz finished?
  const [finished, setFinished] = useState(false)
  // Current question: {id, question: word, options: [def1, def2, def3, def4]}
  const [question, setQuestion] = useState(null)
  ...
  // Results array
  const [results, setResults] = useState([])
  // Previously seen question IDs (to avoid repeats)
  const [seenIds, setSeenIds] = useState([])
  // Timer
  const timer = useTimer()

  /**
   * loadQuestion(excludeIds?): Fetch next vocabulary question
   * Avoids repeating questions by passing previously seen IDs to API
   */
  const loadQuestion = async (excludeIds) => {
    setLoading(true)
    setSelected('')
  ...
  /**
   * submitVocab(option): Submit selected definition and check with API
   */
  const submitVocab = async (option) => {
    ...
    // Get the full user-selected definition from options
    const userDef = question.options[['A','B','C','D'].indexOf(option)]
    // Truncate long definitions for display in results table
  ...
  /**
   * handleSubmitOrNext(): Toggle between submit and next
   * Handles quiz progression (finish when all questions answered)
   */
  const handleSubmitOrNext = async () => {
    if (!question) return
  ...
  // Keyboard shortcuts: 1-4 or a-d instantly select and submit; Enter for submit/next
